...
from utils import evaluate_and_save_metrics, visualize_and_save_predictions, save_loss_curve, dice_score, TverskyLoss
from SegDataset import SegDataset
from model import AttentionUNetResNet18
...
def get_args():
    ...
    parser.add_argument('--seed', type=int, default=42, help='Random seed (1 ~ 100)')
    parser.add_argument('--model', type=str, default='unet', help='Model name')
    parser.add_argument('--data', type=str, default='orig', help='Data name')
...
elif args.data == 'fuzzy':
    img_dir = Path("./MT-Small-Dataset/Benign/Fuzzy_Benign")
    mask_dir = Path("./MT-Small-Dataset/Benign/Ground_Truth_Benign")
fold_idx = args.fold
seed = args.seed
save_root = f'./results/{args.data}/{args.model}/seed_{args.seed}'
...
if args.model == 'attention_unet':
    model = AttentionUNetResNet18(in_ch=3, out_ch=1, pretrained=True).to(device)
    ...
    model = smp.UnetPlusPlus(
        encoder_name="resnet18",         # or "efficientnet-b0"
        encoder_weights="imagenet",
        in_channels=3,
        classes=1,
    )
...
with torch.no_grad():
    for images, masks in test_loader:
        ...
        for i in range(images.size(0)):
            pred_i = preds_sigmoid[i].unsqueeze(0)
            mask_i = masks[i].unsqueeze(0)
            score = dice_score(pred_i, mask_i)
            test_dice += score
            count += 1
